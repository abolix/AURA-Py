...
def GetGame(ID):
    ...
    GameObject = MySQLThread.GetMatch(ID)
    if GameObject != False:
        GoalDetails = {}
    ...
    print('---------------')
    print('---------------')
    MainThread = threading.Timer(5.0, GetGame, args=(ID,))
    MainThread.name = ID
    MainThread.start()
...
def StartProject():
    ...
    for Game in AllData:
        ...
        MainThread = threading.Timer(5.0, GetGame, args=(MatchID,))
        MainThread.name = MatchID
        MainThread.start()
